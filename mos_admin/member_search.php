...
<form method="POST" action="{$_SERVER['PHP_SELF']}">
  ...
  <select name="searchtype">
    <option value="Us_Member" selected>ชื่อ</option>
    <option value="ID_Member">รหัส</option>
  </select><br>
...
function process_form() {
  ...
  while ($row = mysqli_fetch_array($result)) {
    echo "<b>{$i}. ชื่อสมาชิก: {$row['Na_Member']}</b><br>\n";
    echo "รหัสสมาชิก: <a href=\"member_detail.php?ID_Member={$row['ID_Member']}\">{$row['ID_Member']}</a><br>\n";
